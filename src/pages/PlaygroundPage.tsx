...
import { loadProgress, saveProgress, getProgressPercentage } from "@/playground/storage";
...
import { exportLessonZip } from "@/utils/exportZip";
...
export function PlaygroundPage() {
  ...
  const [html, setHTML] = useState(lesson?.starter.html || "");
  const [css, setCSS] = useState(lesson?.starter.css || "");
  ...
  useEffect(() => {
    ...
    const saved = loadProgress(lesson.slug);
    if (saved) {
      setHTML(saved.html);
      setCSS(saved.css);
  ...
  }, [lesson, setSearchParams]);

  // Autosave
  useEffect(() => {
    if (!lesson) return;
    const newCompleted = evaluateAllChallenges(lesson.challenges, debouncedHTML, debouncedCSS);
    ...
    saveProgress(lesson.slug, progress);
  }, [debouncedHTML, debouncedCSS, lesson, manualCompletedChallenges]);

  const handleReset = () => {
    if (!lesson) return;
    setHTML(lesson.starter.html);
    setCSS(lesson.starter.css);
    setCompletedChallenges([]);
    setManualCompletedChallenges([]);
    setResetDialogOpen(false);
    toast.success("Ejercicio reseteado");
  };
  ...
  const handleCopySolutionHTML = () => {
    if (!lesson?.solution?.html) return;
    copyToClipboard(lesson.solution.html, "Solución HTML copiada");
  };

  const handleCopySolutionCSS = () => {
    if (!lesson?.solution?.css) return;
    copyToClipboard(lesson.solution.css, "Solución CSS copiada");
  };

  const handleApplySolution = () => {
    if (!lesson?.solution) return;
    setHTML(lesson.solution.html);
    setCSS(lesson.solution.css);
  ...
  const hasSolution = lesson?.solution?.html || lesson?.solution?.css;
    ...
    <div className="h-[calc(100vh-140px)] flex flex-col">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Button asChild variant="outline" size="sm">
                <Link to={`/lesson/${lesson.slug}/theory`}>
            ...
            <div className="flex items-center gap-3 flex-1 justify-end">
              ...
              <div className="hidden sm:flex items-center gap-3 flex-1 max-w-xs">
                <div className="flex items-center gap-2 text-sm text-neutral-600 min-w-fit">
                  <span>{Array.from(allCompletedChallenges).length}/{lesson.challenges.length}</span>
      ...
      <Dialog open={solutionDialogOpen} onOpenChange={setSolutionDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[80vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>Solución — {lesson.title}</DialogTitle>
            <DialogDescription>
          ...
          <Tabs defaultValue="html" className="flex-1 flex flex-col min-h-0">
            ...
            <TabsContent value="html" className="flex-1 flex flex-col min-h-0">
              ...
              <ScrollArea className="flex-1 border border-neutral-200 rounded-[16px] bg-neutral-50">
                <pre className="p-4 text-sm font-mono text-neutral-900 whitespace-pre-wrap">
                  {lesson.solution?.html || "No hay solución HTML disponible"}
                </pre>
              </ScrollArea>
            </TabsContent>
            ...
            <TabsContent value="css" className="flex-1 flex flex-col min-h-0">
              ...
              <ScrollArea className="flex-1 border border-neutral-200 rounded-[16px] bg-neutral-50">
                <pre className="p-4 text-sm font-mono text-neutral-900 whitespace-pre-wrap">
                  {lesson.solution?.css || "No hay solución CSS disponible"}
                </pre>
              </ScrollArea>
            </TabsContent>
          </Tabs>
